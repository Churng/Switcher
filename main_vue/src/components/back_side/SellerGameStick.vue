...
export default {
  ...
  methods: {
    gameStick: function () {
      const filterStick = this.$root.productsData.filter(item => {
        return item.Category.search('遊戲配件') !== -1
      })
      this.dataStick = filterStick.sort(function (a, b) {
        return a.Price - b.Price
      })
    },
    isRentMethod () {
      const filterStatus = this.dataStick.filter(data => {
        return data.Status.search('可出租') !== -1
      })
      this.$root.productSearchData = filterStatus
    },
    isNew () {
      const filterNew = this.dataStick.sort(function (a, b) {
        return Date.parse(b.PublishDate.replace(/-/g, '/')) - Date.parse(a.PublishDate.replace(/-/g, '/'))
      })
      this.$root.productSearchData = filterNew
    },
    isCheap () {
      const filterPrice = this.dataStick.filter(data => {
    ...
    isAllItem (txt) {
      this.seletTxt = txt
      this.$root.productSearchData = this.dataStick.sort(function (a, b) {
        return a.Price - b.Price
      })
    },
    turnAllItem (txt) {
